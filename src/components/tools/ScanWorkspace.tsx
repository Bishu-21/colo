...
export default function ScanWorkspace() {
  ...
  const savePagesToCache = useCallback((pagesList: ScannedPage[]) => {
    try {
      localStorage.setItem("morpee_cached_pages", encryptString(JSON.stringify(pagesList)));
    } catch (e) {
  ...
  useEffect(() => {
    ...
    // Load cached pages
    try {
      const cached = localStorage.getItem("morpee_cached_pages");
      if (cached) {
  ...
  const rotatePoints90CW = (points: Point[]): Point[] => [
    { x: 1 - points[3].y, y: points[3].x },
    { x: 1 - points[0].y, y: points[0].x },
    { x: 1 - points[1].y, y: points[1].x },
    { x: 1 - points[2].y, y: points[2].x },
  ];

  const rotatePoints90CCW = (points: Point[]): Point[] => [
    { x: points[1].y, y: 1 - points[1].x },
    { x: points[2].y, y: 1 - points[2].x },
    { x: points[3].y, y: 1 - points[3].x },
    { x: points[0].y, y: 1 - points[0].x },
  ];

  const flipPointsH = (points: Point[]): Point[] => [
    { x: 1 - points[1].x, y: points[1].y },
    { x: 1 - points[0].x, y: points[0].y },
    { x: 1 - points[3].x, y: points[3].y },
    { x: 1 - points[2].x, y: points[2].y },
  ];

  const flipPointsV = (points: Point[]): Point[] => [
  ...
  const rotateImage = (direction: "cw" | "ccw") => {
    ...
    img.onload = () => {
      ...
      if (direction === "cw") {
        setCropPoints(rotatePoints90CW(cropPoints));
      } else {
        setCropPoints(rotatePoints90CCW(cropPoints));
      }
    };
  };
  ...
  const flipImage = (axis: "h" | "v") => {
    ...
    img.onload = () => {
      ...
      if (axis === "h") {
        setCropPoints(flipPointsH(cropPoints));
      } else {
        setCropPoints(flipPointsV(cropPoints));
      }
    };
  };

  const updateMagnifier = (point: Point) => {
    if (!originalImgRef.current || !magnifierCanvasRef.current) return;
    const canvas = magnifierCanvasRef.current;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    const img = originalImgRef.current;
    const sourceX = point.x * img.naturalWidth;
    const sourceY = point.y * img.naturalHeight;

    const zoomFactor = 3.5;
    const srcSize = 100 / zoomFactor;
  ...
  useEffect(() => {
    if (draggingPoint !== null && cropPoints[draggingPoint]) {
      requestAnimationFrame(() => {
        updateMagnifier(cropPoints[draggingPoint]);
      });
    }
  }, [draggingPoint, cropPoints]);
  ...
  useEffect(() => {
    if (isCameraActive && streamRef.current && videoRef.current) {
      ...
      videoRef.current.play().catch(err => console.error("Error playing video on render:", err));
    }
  }, [isCameraActive]);
  ...
  const loadScanImageFile = (file: File) => {
    stopCamera();
    if (file.size > 25 * 1024 * 1024) {
      showToast("FILE SIZE LIMIT: The selected file exceeds 25MB. Please upload a smaller image file for client-side stability.", "error");
      return;
    }
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      loadScanImageFile(e.target.files[0]);
    }
  };

  const loadFileFromVault = (file: File) => {
    loadScanImageFile(file);
  };
  ...
  // Apply Document Cleaning Filters (Magic Clean, Grayscale, B&W) and adjust Brightness/Contrast/Saturation
  ...
  // Update canvas preview on Filter mode changes and fine-tuning adjustments
  const [filterWarpedUrl, setFilterWarpedUrl] = useState<string | null>(null);
  useEffect(() => {
    if (warpedUrl) {
      setFilterWarpedUrl(applyFilter(warpedUrl, filterMode, brightness, contrast, saturation));
    }
  }, [warpedUrl, filterMode, brightness, contrast, saturation, applyFilter]);
    ...
    setTimeout(() => {
      ...
      if (lowerName.includes("admit") || lowerName.includes("upsc") || lowerName.includes("roll")) {
        ...
        wordsList = [
          ...
          { text: "MORPEE", x: 150, y: 140, w: 55, h: 18 },
  ...
  // Queue - Add current page to list or save edited page
  const handleSavePage = () => {
    ...
    if (pages.length >= 3 && !isProUnlocked && !editingPageId) {
    ...
    let newPages;
    if (editingPageId) {
      newPages = pages.map(p => p.id === editingPageId ? newPage : p);
  ...
  const compilePdf = async () => {
    ...
    try {
      ...
      pdfDoc.setTitle("MORPEE_SCANNED_DOC");
      pdfDoc.setAuthor("MORPEE_CLIENT");
      pdfDoc.setSubject("Offline scan compilation");
      pdfDoc.setCreator("morpee client side wasm engine");
      ...
      console.error(err);
      showToast("Failed to build PDF. Please check page source coordinates.", "error");
    } finally {
  ...
  return (
    <div className="flex flex-col md:h-[calc(100vh-124px)] md:min-h-0 md:overflow-hidden">
      ...
      <ToolLayout
        className="md:h-[calc(100vh-174px)]"
        sidebar={
          <div className="flex flex-col h-full gap-6">
            ...
            <div className="p-4 bg-primary/10 border border-primary rounded-none font-metadata text-[10px] leading-relaxed tracking-wider text-primary">
              <div className="flex items-center gap-2 font-label-bold text-[11px] mb-1">
                ...
                SECURE HYBRID DOCUMENT SCANNER
              </div>
              Document scanning runs locally in browser RAM. Auth &amp; credits are verified by our secure backend. Safe for Aadhaar &amp; PAN cards.
            </div>
            ...
            {activeStep === "crop" && (
              <section className="space-y-4">
                ...
                <div className="space-y-2">
                  <label className="font-metadata text-[10px] text-secondary uppercase block">Orientation</label>
                  <div className="grid grid-cols-2 gap-1.5">
                    <button
                      type="button"
                      onClick={() => rotateImage("cw")}
                      className="py-2 px-1 border border-carbon text-[10px] uppercase font-label-bold flex items-center justify-center gap-1 hover:bg-carbon hover:text-white transition-colors"
                    >
                ...
                {/* Presets / Resets */}
                <div className="space-y-2">
                  <label className="font-metadata text-[10px] text-secondary uppercase block">Crop Presets</label>
                  <div className="grid grid-cols-3 gap-1">
                    <button
                      ...
                      className="py-1 px-1 border border-carbon/25 text-[9px] uppercase hover:bg-carbon hover:text-white transition-colors"
                    >
                      Full Page
                    </button>
                    <button
            ...
            {activeStep === "filter" && (
              <section className="space-y-4">
                ...
                <div className="space-y-4 pt-4 border-t border-outline-variant">
                  <h3 className="font-metadata text-metadata text-secondary uppercase">Fine-Tuning Adjustments</h3>
                  <div>
                    <div className="flex justify-between text-[11px] font-metadata mb-1">
                  ...
                  <button
                    ...
                    className="text-[10px] text-primary hover:underline uppercase block text-right font-label-bold w-full"
                  >
                    Reset Adjustments
                  </button>
                </div>
              ...
              <div ref={cropWrapperRef} className="relative max-h-[85%] max-w-[85%] aspect-auto shadow-2xl pointer-events-none">
                ...
                {draggingPoint !== null && (
                  <div
                    style={{
                      position: 'absolute',
                      left: `${cropPoints[draggingPoint].x * 100}%`,
                      top: `${cropPoints[draggingPoint].y * 100}%`,
                          ...
                          <div className="flex gap-0.5">
                            <button
                              onClick={() => {
                                ...
                                setUploadedImage(p.originalUrl);
                                setWarpedUrl(p.warpedUrl);
                                setCropPoints(p.cropPoints);
                                setFilterMode(p.filter);
                                setBrightness(p.brightness || 0);
                                setContrast(p.contrast || 0);
                                setSaturation(p.saturation || 0);
                                setOcrText(p.ocrText);
                                setOcrWords(p.ocrWords);
                                setActiveStep("crop");
                              }}
                              className="p-1 border border-primary text-primary hover:bg-primary hover:text-white cursor-pointer flex"
                              title="Edit Cropping & Filters"
                            >
                    ...
                    <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        onClick={async () => {
                          if (pdfResultUrl) {
                            try {
                              ...
                              const compiledFile = new File([blob], `scanned_doc_${Date.now()}.pdf`, { type: "application/pdf" });
                              await saveFileToVault(compiledFile);
                              showToast("Success: Scanned PDF saved to Local Sandbox Vault!", "success");
                            } catch (err) {
                              console.error(err);
                              showToast("Error: Failed to save scanned PDF to local vault.", "error");
                            }
                          }
                        }}
                        className="py-3 px-4 border border-primary bg-primary/5 text-primary font-label-bold text-xs uppercase hover:bg-primary hover:text-white transition-all flex items-center justify-center gap-1.5"
                      >
                        Save to Vault
                      ...
                      <button
                        onClick={() => {
                          setPdfResultUrl(null);
                          setPages([]);
                          localStorage.removeItem("morpee_cached_pages");
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 select-none">
          <div className="bg-white border border-carbon max-w-md w-full p-8 shadow-2xl relative">
            ...
            <div className="text-center space-y-4">
              ...
              <h3 className="font-headline-sm text-lg uppercase">Unlock Morpee Pro Scanner</h3>
              ...
              <div className="pt-4 space-y-2">
                <Link
                  href="/billing"
                  onClick={() => setShowUpgradeModal(false)}
                  className="block w-full py-4 bg-primary text-white font-label-bold text-xs uppercase hover:bg-muted-teal transition-all cursor-pointer shadow-md text-center"
                >
